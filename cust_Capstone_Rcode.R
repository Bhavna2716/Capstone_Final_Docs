...
cust.spend.plot <- ggplot(cust.df, aes(x = online.spend, y = store.spend, col = cust.category)) + 
                  geom_point(size = 2)
cust.spend.plot 
...
### DISTANCE TO STORE AND TRANSECTIONS 
...
#Online trasection mapped to online variables 
...
cust.sat.selection <- ggplot(cust.df, aes(x = sat.service, fill = cust.category)) + geom_bar()
cust.sat.selection
cust.sat.selection
